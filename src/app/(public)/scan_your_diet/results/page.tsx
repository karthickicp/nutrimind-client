...
const macros = [
    ...
    { label: "Calories", value: "2,230 kcal", percentage: 33, color: "text-red-500", borderColor: "border-red-500" },
    { label: "Protein", value: "23%", subValue: "/100%", percentage: 23, color: "text-cyan-500", borderColor: "border-cyan-500" },
];

const ingredients = [
    { name: "Grilled Chicken Strips", cal: "220 kcal", p: "30g", c: "0g", f: "5g" },
    { name: "Naan Bread", cal: "220 kcal", p: "30g", c: "0g", f: "5g" },
    { name: "Sautéed Bell Peppers (Red & Yellow)", cal: "220 kcal", p: "30g", c: "0g", f: "5g" },
    ...
    return (
        <main className="flex-1 flex flex-col p-4 md:p-8 relative container mx-auto max-w-6xl">
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <Button variant="outline" size="icon" className="rounded-full bg-white/5 border-white/10 hover:bg-white/10 w-10 h-10">
            ...
            <div className="flex flex-col lg:flex-row w-full bg-[#1E1E1E] rounded-3xl overflow-hidden shadow-2xl min-h-[600px]">
                ...
                <div className="flex-1 p-6 md:p-8 flex flex-col gap-6 bg-[#1A1A1A]">
                    <div className="flex-1 flex flex-row gap-6">
                        ...
                        <div className="flex-1">
                            <h1 className="text-xl md:text-2xl font-bold leading-tight">
                                Grilled Chicken Strips served over Quinoa with Roasted Seasonal Vegetables
                            </h1>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 justify-items-center mt-auto">
                        {macros.map((m, i) => (
                            <div key={i} className="flex flex-col items-center justify-center bg-[#252525] rounded-full w-28 h-28 relative border-4 border-[#333]">
                                <div className={`absolute inset-0 rounded-full border-4 ${m.borderColor} border-t-transparent -rotate-45`} style={{ opacity: 0.8 }}></div>

                                <span className="text-gray-400 text-xs">{m.label}</span>
                                <span className={`text-lg font-bold ${m.color}`}>{m.value}</span>
                                {m.subValue && <span className="text-[10px] text-gray-500">{m.subValue}</span>}
                            </div>
                        ))}
                    </div>

                    <Button className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-6 text-lg rounded-xl mt-4">
                        Okay
                    </Button>
                </div>
